...
class GameBoard:
    ...
    def check_nextplayer(self, bd: list[list[int]]):
    ...
    def minmax(self, bd: list[list[int]], depth=0):
        ...
        moves = [
            (r, c) for r in range(3) for c in range(3) if bd[r][c] == 0
        ...
        if player == 1:  # x's turn, maximize
            ...
            for r, c in moves:
                ...
                if (
                    score > best
                ):  # pick the move with the largest score, update the best move!!
                    best, move = score, (r, c)
            return move, best
            ...
            for r, c in moves:
                ...
                if (
                    score < best
                ):  # pick the move with the smallest score, update the best move!!
                    best, move = score, (r, c)
            return move, best

    def alphabeta(
        self, bd: list[list[int]], depth: float | int, alpha=-1e10, beta=1e10
    ):
        self.ab_nodes += 1
        result = self.checkwin(bd)
        if result == 1:
...
class TicTacToeGame:
    ...
    def playturn(self):
        ...
        if self.turn == 1:
            ...
            # I call this the "play stupid games, win stupid prizes" solution
            humanrow, humancol = map(
                lambda x: x % 3,
                map(int, map(str.strip, user_input.split(","))),  # pyright: ignore[reportPossiblyUnboundVariable]
            )
            ...
            move, score = self.gameboard.alphabeta(
                self.gameboard.entries, self.alpha, self.beta
            )
